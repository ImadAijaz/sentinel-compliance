...
const FILE_RULES = {
  ...
  // Staff workbook headings use shorter labels than the provider roster.
  "ACLS": "\\bacls\\b", "ATLS": "\\batls\\b", "PALS": "\\bpals\\b", "BLS": "\\bbls\\b",
  "BLS(Exp Date)": "\\bbls\\b", "RN License": "rn licen[cs]e|board of nursing|\\brn\\b.*lic",
  "Nursys Verification": "nursys", "Flu": "\\bflu\\b|influenza", "Flu (Exp Date)": "\\bflu\\b|influenza",
  "TB": "\\btb\\b|ppd|tubercul|quantiferon", "TB (Exp Date)": "\\btb\\b|ppd|tubercul|quantiferon",
  "EMT / CMA Certification": "\\bemt\\b|\\bcma\\b", "EMT/MRT": "\\bemt\\b|\\bmrt\\b",
  "Nursing Diploma": "nursing diploma|degree", "Diploma HS or College": "diploma|degree",
  "SS Card": "social security|ss card", "Social Security Card": "social security|ss card",
  "Background Check": "background", "I-9": "\\bi[ -]?9\\b", "i-9": "\\bi[ -]?9\\b",
  "Signed Handbook": "handbook", "Signed P&P": "polic|procedure|p[ &]*p",
...
// Destination phase for a provider credential uploaded through Sentinel.  This mirrors the
// local master sync's filing policy, but uses the tracked category because a phone upload's
// original filename may be just "scan.pdf".
function providerPhaseForCategory(category) {
  const c = String(category || "").toLowerCase();
  if (/oig|sam|npdb|exclusion|background|sanction/.test(c)) return 2;
  if (/verify|verification|primary source/.test(c)) return 1;
  return 0;
}

module.exports = { FILE_RULES, normf, matchByRules, providerPhaseForCategory };
