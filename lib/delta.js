...
async function applyRosterDelta(items) {
  ...
  try {
    ...
    // Count UNIQUE provider keys — a delta written by an older build could repeat a key once per
    // credential row, which would make this look like a mass deletion when it is not.
    const removedHit = new Set((delta.removed || []).filter(k => bakedKeys.has(k))).size;
    // 60%, not 30%: the baked data.json is a point-in-time bake and the workbook is edited daily,
    // so a fair number of providers legitimately disappear from it over time. Only a near-total
    // mismatch means the roster misparsed. Too tight a threshold here silently suppressed the
    // whole provider merge, which hid every newly added provider.
    const removesTooMany = bakedKeys.size > 0 && removedHit > bakedKeys.size * 0.6;
    const deltaCorrupt = newBad || removesTooMany;
    // Surface the suppression to callers. Silently falling back to baked data is exactly how a
    // stale board went unnoticed: newly added providers simply never appeared and nothing said
    // why. Attached to the returned array so no call site's signature changes.
    if (deltaCorrupt) {
      out.deltaSuppressed = {
        reason: newBad
          ? junkCount + " of " + newList.length + " new provider names read as column labels, so the roster layout has probably changed"
          : removedHit + " of " + bakedKeys.size + " known providers would have been dropped at once",
    ...
    if (!deltaCorrupt) {
      ...
      newList.forEach(p => {
        // Older roster deltas may still contain Excel rich-text/hyperlink objects. Normalize
        // them while reading so deploying this fix removes the phantom card immediately,
        // without waiting for the next roster regeneration.
        const last = cellText(p.last).replace(/[\*,()]+/g, "").trim();
        const first = cellText(p.first).trim();
        ...
        if (!last || isJunkName(last) || isJunkName(first)) return;
        const ekey = slug(last, first);
        if (existingKeys.has(ekey)) return;
        const entity = (first + " " + last).trim();
    ...
    // Contact details from the COI Roster sheet (degree/title + email). Applied to every one of a
    // provider's items so any view can read them off the first item it has. Safe regardless of the
    // corruption guard above: these only ADD fields, they never remove or reassign a provider.
    if (delta.contacts && typeof delta.contacts === "object") {
      out.forEach(i => {
        if (i.scope !== "provider") return;
        const c = delta.contacts[i.entityKey];
        if (!c) return;
        if (c.email && !i.email) i.email = c.email;
        if (c.title) i.providerTitle = c.title;
      });
    }
