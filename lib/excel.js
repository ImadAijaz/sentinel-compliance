...
// The master roster can now be moved independently into the corporate SharePoint drive.  Until
// MS_ROSTER_DRIVE_ID/MS_ROSTER_PATH are set, the existing personal-OneDrive location remains the
// fallback, so deploying this support does not interrupt today's live roster.
const SOURCE_DRIVE_ID = process.env.MS_DRIVE_ID || "b!hICmGNzaFEiC8Z6vebrpNWzB937MR0tFsLlTxA2x3Z9-nxsW_blJTrLUhaL3IsBm";
const ROSTER_DRIVE_ID = process.env.MS_ROSTER_DRIVE_ID || SOURCE_DRIVE_ID;

function rosterRoot() { return GRAPH + "/drives/" + ROSTER_DRIVE_ID + "/root:/" + encPath(ROSTER_PATH); }
...
// ---- Provider contact details from the "WCGTX COI Roster" sheet ---------------------------
// That sheet carries Title (MD / DO / PA / NP) and Email Address, which nothing else in the
// workbook holds — the Credentials sheet has neither. Columns are found by HEADER, same as
// everywhere else, so an inserted column can't shift them.
// NOTE: there is no phone column anywhere in this workbook.
const COI_SHEET = "WCGTX COI Roster";
async function readCoiContacts(token) {
  let sh;
  try { sh = await readSheet(token, COI_SHEET); } catch (e) { return []; }
  const header = (sh.values || [])[0] || [];
  const labels = header.map(h => cellText(h).replace(/\s+/g, " ").trim().toLowerCase());
  const { lastIdx, firstIdx } = detectNameCols(header);
  const findCol = re => { const i = labels.findIndex(l => re.test(l)); return i; };
  const titleIdx = findCol(/^title$/);
  const emailIdx = findCol(/e-?mail/);
  const out = [];
  (sh.values || []).slice(1).forEach(r => {
    const last = cellText(r[lastIdx]).replace(/[\*,()]+/g, "").trim();
    const first = cellText(r[firstIdx]).trim();
    if (!last) return;
    const title = titleIdx >= 0 ? cellText(r[titleIdx]).trim() : "";
    let email = emailIdx >= 0 ? cellText(r[emailIdx]).trim() : "";
    if (email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) email = "";
    if (!title && !email) return;
    out.push({ last, first, title, email });
  });
  return out;
}
// Match a COI row to a Credentials-sheet provider key. The two sheets spell first names
// differently ("Rahman Mohammed" vs "Rahman M"), so an exact slug match is not enough:
// fall back to surname + first initial, and finally surname alone when it is unambiguous.
function matchContactsToKeys(contacts, providerKeys) {
  const keys = [...providerKeys];
  const bySlug = new Map(keys.map(k => [k, k]));
  const norm = s => String(s || "").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "");
  const lastCount = {};
  keys.forEach(k => { const l = k.split("-")[0]; lastCount[l] = (lastCount[l] || 0) + 1; });
  const out = {};
  for (const c of contacts) {
    const lastN = norm(c.last), firstN = norm(c.first);
    let key = null;
    const exact = lastN + (firstN ? "-" + firstN : "");
    if (bySlug.has(exact)) key = exact;
    if (!key) {
      const initial = firstN ? firstN[0] : "";
      const cands = keys.filter(k => k.split("-")[0] === lastN);
      if (cands.length === 1) key = cands[0];
      else if (cands.length > 1 && initial) {
        const narrowed = cands.filter(k => { const parts = k.split("-"); return parts[1] && parts[1][0] === initial; });
        if (narrowed.length === 1) key = narrowed[0];
      }
    }
    if (key) out[key] = { title: c.title || undefined, email: c.email || undefined };
  }
  return out;
}
...
const _STAFF = "https://wcgtx.sharepoint.com/sites/CorporateArchivesDirectory/Shared%20Documents/Sama%20Farooqui/Sentinel/Staff";
const STAFF_FILES = [
  { file: "WCGTX Phyicians_04.08.2020/..WCGTX Master Rosters/CHER RN and FD Roster and Credentialing log.xlsx",
    facility: "Castle Hills ER", folder: _SR + "/Castle%20Hills", staffDir: "Castle Hills",
  ...
  { file: "WCGTX Phyicians_04.08.2020/..WCGTX Master Rosters/Frisco ER RN and FD Roster and Credentialing.xlsx",
    facility: "Frisco ER", folder: _SR + "/Frisco", staffDir: "Frisco",
...
async function downloadWorkbookAt(token, path) {
  const r = await fetch(GRAPH + "/drives/" + SOURCE_DRIVE_ID + "/root:/" + encPath(path) + ":/content", { headers: { Authorization: "Bearer " + token } });
  if (!r.ok) return null;
...
module.exports = {
  ...
  cellText, cellDate, expiryDatesFromValues, buildStaffItems, readCoiContacts, matchContactsToKeys,
