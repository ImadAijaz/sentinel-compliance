...
const PROVIDER_COOKIE = "sentinel_provider";
...
function getSession(req) {
  ...
  // Provider links use the same signing key but a different cookie and token type. Never let a
  // provider-link token become a staff session, even if somebody manually moves the cookie.
  return session && session.type !== "provider" ? session : null;
}
...
function signProviderAccess(entityKey, maxAgeSec) {
  const key = String(entityKey || "").trim();
  if (!key) throw new Error("provider key is required");
  const seconds = Math.max(60, Math.min(Number(maxAgeSec) || 0, 60 * 60 * 24 * 30));
  return sign({ type: "provider", entityKey: key, exp: Date.now() + seconds * 1000 });
}
function verifyProviderAccess(token) {
  const session = verify(token);
  return session && session.type === "provider" && typeof session.entityKey === "string" && session.entityKey
    ? session : null;
}
function getProviderSession(req) { return verifyProviderAccess(parseCookies(req)[PROVIDER_COOKIE]); }
function providerCookieHeader(token, maxAgeSec) {
  return PROVIDER_COOKIE + "=" + token + "; HttpOnly; Secure; SameSite=Lax; Path=/; Max-Age=" + maxAgeSec;
}
function clearProviderHeader() { return PROVIDER_COOKIE + "=; HttpOnly; Secure; SameSite=Lax; Path=/; Max-Age=0"; }
...
module.exports = {
  sign, verify, getSession, cookieHeader, clearHeader, isCronRequest, COOKIE,
  signProviderAccess, verifyProviderAccess, getProviderSession, providerCookieHeader,
  clearProviderHeader, PROVIDER_COOKIE,
};
