...
function classify(file, resolve) {
  ...
  // Urgent-care staff are filed directly below the site, not inside a Nursing Staff group.
  // This shape was verified in the live source inventory. Require the unambiguous comma name.
  if (/^Urgent Care /.test(site) && p.length >= 2 && /^[^,]+,\s*\S/.test(p[1])) {
    return { family: "staff", entity: site, base: ["Staff", site, p[1], category(p[2])].join("/"), deeper: p.slice(3), name };
  }
