...
const ARCHIVE_SEG = /^[.\s]*(z+[._ ]?)*(expired|archive|archived|inactive|old|superseded)\b|^z+archive|\bexpired$/i;
function isArchivedSegment(seg) {
  return ARCHIVE_SEG.test(String(seg || "").trim().replace(/_/g, " "));
}
...
function classifySection(relFolder, fileName) {
  const hay = (String(relFolder || "") + "/" + String(fileName || "")).toLowerCase().replace(/[_-]+/g, " ");
...
// Recurring operational obligations are different from ordinary facility records.  A QAPI
// minute does not itself expire, but it is evidence that the monthly QAPI obligation was met;
// the NEXT meeting is then due one month later.  Keep this policy beside the facility filing
// rules so the local disk refresh and the Graph delta scanner cannot drift apart.
//
// Cadences below are only asserted where the category or the governing rule is explicit:
// QAPI monthly (26 TAC 509.63), governing body annually (509.41), fire drill quarterly per
// shift (509.121), and annual fire inspection (509.64).  Vendor/service rows without a stated
// cadence still refresh their latest evidence link/date, but are not given an invented due date.
const RECURRING_RULES = [
  { category: "Inspection — Generator Semi-Annual", match: /generator|load test/i, months: 6 },
  { category: "Inspection — Generator Annual", match: /generator|load test/i, months: 12 },
  { category: "Inspection — Back Flow Test", match: /back\s*flow/i, months: 12 },
  { category: "Inspection — Fire Inspection", match: /fire (inspection|marshal)/i, exclude: /alarm|sprinkler|extinguisher|drill/i, months: 12 },
  { category: "Inspection — Fire Sprinklers", match: /sprinkler/i, months: 12 },
  { category: "Inspection — Fire Alarm Inspection", match: /(firetrol.*alarm|fire alarm|alarm inspection)/i, exclude: /permit/i, months: 12 },
  { category: /^Inspection — Fire Extinguishers(?:\s|$)/, match: /extinguisher/i, months: 12 },
  { category: /^Inspection — Alarm Permit(?:\s|#|$)/, match: /alarm permit/i, dateIsDue: true },
  { category: "Inspection — Fire drill", match: /fire drill/i, months: 3, note: "One drill per defined shift per quarter" },
  { category: "Meeting — Governing Board Meeting", match: /govern(?:ing)? board.*(?:meeting|mtg|minutes|agenda)|(?:meeting|mtg|minutes|agenda).*govern(?:ing)? board/i, months: 12 },
  { category: "Meeting — QAPI", match: /\bqapi\b|quality (?:assurance|assessment).*performance improvement/i, months: 1 },
  { category: "Inspection — SPBS Equipment Maintenance", match: /\bspbs\b|6\s*mo(?:nth)?\s*service/i, months: 6 },
  // Evidence-only rows: refresh the latest document and completion date, but do not manufacture
  // a regulatory deadline where the source category does not state one.
  { category: "Inspection — Biogenic Solutions", match: /biogenic/i },
  { category: /^Meeting — Staff meeting RNs?$/i, match: /\b(?:rn|nurs(?:e|ing))\b.*(?:meeting|mtg)|(?:meeting|mtg).*\b(?:rn|nurs(?:e|ing))\b/i },
  { category: "Meeting — Staff Meeting Front Desk", match: /front (?:desk|staff).*(?:meeting|mtg)|(?:meeting|mtg).*front (?:desk|staff)/i },
  { category: "Meeting — Medical Staff meeting", match: /medical staff.*(?:meeting|mtg)|(?:meeting|mtg).*medical staff/i },
  { category: "Meeting — Providers & RNs", match: /provider.*\brn\b|\brn\b.*provider/i },
  { category: "Meeting — State Inspections", match: /state inspection/i },
];

function recurringRule(category) {
  const c = String(category || "");
  return RECURRING_RULES.find(r => typeof r.category === "string" ? r.category.toLowerCase() === c.toLowerCase() : r.category.test(c)) || null;
}

function addMonths(iso, months) {
  if (!iso || !months) return iso || null;
  const p = String(iso).match(/^(\d{4})-(\d{2})-(\d{2})$/); if (!p) return null;
  const y = +p[1], m = +p[2], d = +p[3];
...
// Return every recurring row that a document is evidence for.  Generator evidence legitimately
// satisfies both the annual and semi-annual cards, so this deliberately returns an array.
function matchRecurringObligations(items, relPath, fileName) {
  // Match meaning from the FILE name, not generic parent folders such as "Fire & Alarm
  // Inspections"; using the whole path made an ordinary fire-marshal report falsely satisfy
  // the fire-alarm card merely because both were stored under that shared folder.
  const hay = String(fileName || relPath || "").replace(/[_\-.]+/g, " ").replace(/\s+/g, " ");
  // A scheduled agenda, quote or invoice does not prove an obligation was completed.
  if (/\bagenda\b|\binvit(?:e|ation)\b|\bschedule\b|\bquote\b|\bestimate\b|\bproposal\b|\binvoice\b/i.test(hay)) return [];
  const recordDate = require("./graph").dateFromName(String(fileName || ""));
  const out = [];
  for (const item of (items || [])) {
    if (!item || item.scope !== "other") continue;
    const rule = recurringRule(item.category); if (!rule) continue;
    if (!rule.match.test(hay) || (rule.exclude && rule.exclude.test(hay))) continue;
    const nextDue = recordDate ? (rule.dateIsDue ? recordDate : (rule.months ? addMonths(recordDate, rule.months) : null)) : null;
    out.push({ item, recordDate, nextDue, cadenceMonths: rule.months || null, dateIsDue: !!rule.dateIsDue, note: rule.note || null });
  }
  return out;
}
...
module.exports = {
  ...
  isNonExpiring, RECURRING_RULES, recurringRule, addMonths, matchRecurringObligations,
};
