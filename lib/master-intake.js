...
async function get(token, url, missing) {
  ...
  if (!r.ok) throw new Error("Microsoft HTTP " + r.status + (r.status === 429 ? ": rate limited; progress saved, retry later" : " while reading document metadata"));
  return r.json();
}
...
async function lock(token, namespace) {
  ...
  await G.ensureFolderIn(token, G.docsRoot(), BASE + "/_State");
...
function report(state) {
  ...
  return {
    ok: !state.error, stage: state.stage, error: state.error || null,
...
async function inventory(token, state, deadline) {
  for (const side of ["source", "destination"]) {
    ...
    while (inv.queue.length && Date.now() < deadline) {
      const batch = inv.queue.slice(0, 6);
...
async function copies(token, state, deadline) {
  ...
  while (todo.length && Date.now() < deadline) {
    await Promise.all(todo.splice(0, 5).map(async op => {
      try { await copyOne(token, op, cache); }
      catch (e) { op.status = "error"; op.error = String(e.message || e); }
    }));
  }
