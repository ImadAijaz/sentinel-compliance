...
function namesFrom(sh) {
  ...
  return (sh.values || []).slice(1)
    .map(r => ({
      // Excel cells can be rich text or hyperlinks ({text, hyperlink}). String(object)
      // produced the phantom provider "[object Object] Carpenter" on the dashboard.
      last: xl.cellText(r[lastIdx]).replace(/[\*,()]+/g, "").trim(),
      first: xl.cellText(r[firstIdx]).trim(),
    }))
    .filter(x => x.last);
}
...
async function regenerateRoster(token) {
  ...
  // Contact details (degree/title + email) live only on the COI Roster sheet. Best-effort: a
  // problem reading that sheet must not fail the roster refresh.
  let contacts = {};
  try {
    const rows = await xl.readCoiContacts(token);
    const liveKeys = new Set([...liveActiveKeys, ...liveInactiveKeys, ...seedKeys]);
    contacts = xl.matchContactsToKeys(rows, liveKeys);
  } catch (ce) { contacts = {}; }
