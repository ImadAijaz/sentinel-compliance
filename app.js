...
(function () {
  ...
  function docTile(it, showEntity) {
    ...
    // A dated record (meeting minutes, an inspection report, an "as of" snapshot) reads as the
    // record it is, dated, rather than borrowing the vocabulary of a credential.
    const badge = it.datedRecord ? [it.archivedCopy ? "superseded" : "record", "b-ready"] : (bmap[s.key] || [s.label, "b-prog"]);
    ...
    t.innerHTML =
      ...
      '<div class="c-foot"><span class="l">' + (it.expires ? fmtD(it.expires)
        : (it.recordDate ? (it.archivedCopy ? "was " : "") + fmtD(it.recordDate)
        : (it.permanent ? "No expiry" : ((it.isFile === false && !it.centralProof) ? "Awaiting upload" : "—")))) + '</span><span class="doc-fr">' + proof + mail + '</span></div>';
